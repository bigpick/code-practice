...
def random_point_in_circle_numpy():
    return numpy.random.uniform(-1., 1.)**2 + numpy.random.uniform(-1., 1.)**2 <= 1
...
if __name__ == "__main__":
    for TO_GENERATE in [1, 10, 100, 1000, 10_000, 100_000, 1_000_000, 10_000_000]: # 100_000_000, 1_000_000_000]:
        in_circle = 0
        for _ in range(TO_GENERATE):
            if random_point_in_circle():
                in_circle += 1

        print(f"Rough estimate of pi based on {TO_GENERATE} intervals: {4.0*(float(in_circle)/float(TO_GENERATE)):.3f}")
